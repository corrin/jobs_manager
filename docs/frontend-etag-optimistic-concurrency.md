...
- Delivery receipt ingestion: `DeliveryReceiptRestView.post()` now requires `If-Match` and refreshes the PO ETag after processing.
- Service layer checks: `PurchasingRestService.update_purchase_order` and `process_delivery_receipt` both perform `select_for_update` and raise `PreconditionFailedError` on ETag mismatch.
...
- Include the same header when posting delivery receipts to `/purchasing/rest/delivery-receipts/` (the body still carries `purchase_order_id` and allocations).
...
### UI handling recommendations
- When receiving `412`, display a message such as: “This purchase order was updated elsewhere. We reloaded the latest data; please review and resubmit.” Apply the same flow after a delivery receipt submission fails with `412`.
- Disable the submit button while a PATCH or delivery receipt POST is in-flight to avoid accidental duplicate requests.
...
- Service layer now enforces optimistic concurrency via `PurchasingRestService.update_purchase_order` and `process_delivery_receipt`.
