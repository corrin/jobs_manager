...
const findLatestCaptures = () => {
  ...
  if (!uatFiles.length || !prodFiles.length) {
    console.error('Missing capture files. Run capture_metrics.js for both UAT and production first.');
    process.exit(1);
  }
...
const analyze = () => {
  ...
  const uat = JSON.parse(fs.readFileSync(path.join(process.cwd(), files.uat), 'utf8'));
  const prod = JSON.parse(fs.readFileSync(path.join(process.cwd(), files.production), 'utf8'));
