...
class JSONToMySQLConverter:
    def __init__(self):
        ...
        self.datetime_fields = {
            'action_time', 'applied', 'completed_at', 'created_at', 'created_date_utc',
            'date', 'date_joined', 'django_created_at', 'django_updated_at', 
            'expires_at', 'expire_date', 'history_date', 'last_login',
            'last_xero_deep_sync', 'last_xero_sync', 'next_run_time',
            'quote_acceptance_date', 'run_time', 'started_at', 'timestamp',
            'updated_at', 'uploaded_at', 'xero_last_modified', 'xero_last_synced',
            'delivery_date', 'expected_delivery', 'order_date', 'parsed_at'
        }
        
        # JSON fields that need proper JSON formatting
        self.json_fields = {
            'raw_json', 'raw_ims_data', 'additional_contact_persons', 'all_phones'
        }
    ...
    def escape_sql_value(self, value: Any, field_name: str = None) -> str:
        ...
        if value is None:
            ...
            if field_name in self.json_fields:
                ...
                logging.warning(f"Unexpected dict/list value for non-JSON field '{field_name}': {type(value)} - {str(value)[:100]}...")
                return 'NULL'
    ...
    def convert_record(self, record: Dict) -> List[str]:
        ...
        for field_name, field_value in fields.items():
            ...
            if (table_name in self.m2m_mappings and 
                field_name in self.m2m_mappings[table_name]):
        ...
        main_insert = f"INSERT INTO `{table_name}` ({columns_str}) VALUES ({values_str});"
            ...
            for field_name, related_ids in m2m_fields.items():
                ...
                for related_id in related_ids:
                    if field_name == 'archived_pricings':
                        m2m_insert = (f"INSERT INTO `{m2m_table}` "
                        ...
                        print(f"Warning: Unknown M2M field {field_name}", file=sys.stderr)
                        continue
