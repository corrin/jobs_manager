...
const captureMetrics = async (env) => {
  loadEnv(env);
  ...
  const jobId = process.argv[3] || 'faf1bdaa-a21b-45c4-96c8-57eecbc505d4';
  ...
  try {
    ...
    // Save results
    const outputFile = `capture_${env}_${Date.now()}.json`;
...
if (!['production', 'uat'].includes(env)) {
  console.log('Usage: node scripts/capture_metrics.js [production|uat] [job-id]');
  process.exit(1);
}
