...
class ChatAPIEndpointTests(TestCase):
    ...
    def setUp(self):
        ...
        self.chat_history_url = reverse(
            'job-quote-chat-history', 
            kwargs={'job_id': str(self.job.id)}
        )
        self.chat_interaction_url = reverse(
            'job-quote-chat-interaction', 
            kwargs={'job_id': str(self.job.id)}
        )
    ...
    def test_chat_history_delete_message(self):
        ...
        self.assertFalse(
            JobQuoteChat.objects.filter(id=message.id).exists()
        )
    ...
    def test_chat_history_job_not_found(self):
        ...
        url = reverse(
            'job-quote-chat-history',
            kwargs={'job_id': non_existent_job_id}
        )
    ...
    @patch('apps.job.services.gemini_chat_service.GeminiChatService.generate_ai_response')
    def test_chat_interaction_success(self, mock_generate_response):
        ...
        self.assertEqual(response.data['data']['content'], 'AI response to user message')
        self.assertEqual(response.data['data']['role'], 'assistant')
        
        # Verify service was called with correct parameters
        mock_generate_response.assert_called_once_with(
            job_id=str(self.job.id),
            user_message='Test user message for AI'
        )
