...
class JobQuoteChatModelTests(TestCase):
    ...
    def test_string_representation(self):
        ...
        expected_str = f"JobQuoteChat(job={self.job.name}, role=user, message_id=test-message-3)"
    ...
    def test_metadata_json_serialization(self):
        ...
        self.assertEqual(retrieved_message.metadata["nested"]["level1"]["level2"], ["item1", "item2"])
...
class JobQuoteChatQueryTests(TestCase):
    ...
    def test_order_by_timestamp(self):
        ...
        for i in range(9):
            self.assertLessEqual(messages_asc[i].timestamp, messages_asc[i+1].timestamp)
        
        # Verify descending order
        for i in range(9):
            self.assertGreaterEqual(messages_desc[i].timestamp, messages_desc[i+1].timestamp)
    ...
    def test_recent_messages_limit(self):
        ...
        recent_messages = JobQuoteChat.objects.filter(job=self.job).order_by('-timestamp')[:5]
    ...
    def test_count_by_role(self):
        ...
        role_counts = JobQuoteChat.objects.filter(job=self.job).values('role').annotate(count=Count('role'))
    ...
    def test_metadata_queries(self):
        ...
        messages_with_model = [
            ...
            if msg.metadata.get('model') == 'gemini-pro'
        ]
