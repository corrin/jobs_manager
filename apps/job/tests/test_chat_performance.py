...
class ChatPerformanceTests(TransactionTestCase):
    ...
    def test_concurrent_chat_sessions(self):
        ...
        for i, job in enumerate(jobs):
            thread = threading.Thread(
                ...
                args=(str(job.id), f"Concurrent message {i}")
            )
    ...
    def test_tool_execution_performance(self):
        ...
        with patch.object(self.service, 'get_gemini_client') as mock_client:
            ...
            with patch.object(self.service, '_execute_mcp_tool', return_value="Tool result"):
                start_time = time.time()
                result = self.service.generate_ai_response(str(self.job.id), "Search products")
                end_time = time.time()
    ...
    def test_streaming_response_simulation(self):
        ...
        with patch.object(self.service, 'get_gemini_client') as mock_client:
            ...
            start_time = time.time()
            result = self.service.generate_ai_response(str(self.job.id), "Generate long response")
            end_time = time.time()
    ...
    def test_metadata_storage_performance(self):
        ...
        large_metadata = {
            ...
            "chat_history": [
                {"role": "user", "content": f"History message {i}"}
                for i in range(50)
            ]
        }
...
class ChatLoadTests(TestCase):
    ...
    def test_search_performance(self):
        ...
        for term in search_terms:
            results = JobQuoteChat.objects.filter(
                job=job,
