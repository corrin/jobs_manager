...
class PaidFlagService:
    ...
    def update_paid_flags(dry_run: bool = False, verbose: bool = False) -> PaidFlagResult:
        ...
        if verbose:
            logger.info(f"Found {completed_jobs.count()} completed jobs not marked as paid")
        ...
        for job in completed_jobs:
            ...
            if not has_invoice:
                missing_invoices += 1
                if verbose:
                    logger.info(f"Job {job.job_number} - {job.name} has no associate invoice")
                continue
            
            if not invoice.paid:
                unpaid_invoices += 1
                if verbose:
                    logger.info(f"Job {job.job_number} - {job.name} has unpaid invoice {invoice.number}")
            else:
                if verbose:
                    logger.info(f"Job {job.job_number} - {job.name} has paid invoice {invoice.number}")
                if dry_run:
