...
class PDFDataValidationService:
    ...
    def validate_extracted_data(self, data: Dict[str, Any]) -> Tuple[bool, List[str], List[str]]:
        ...
        if valid_items == 0 and items:
            ...
            self.warnings.append(f"Only {valid_items} out of {len(items)} items are valid")
    ...
    def _validate_single_item(self, item: Dict[str, Any], index: int) -> bool:
        ...
        if not product_name and not description:
            self.validation_errors.append(f"Item {index}: Missing both product name and description")
            item_valid = False
        ...
        if unit_price is not None:
            try:
                ...
                if normalized_price is not None and normalized_price < 0:
                    self.warnings.append(f"Item {index}: Negative price {normalized_price}")
            except ValueError as e:
                self.warnings.append(f"Item {index}: Invalid price format '{unit_price}': {e}")
        ...
        if product_name and len(product_name) > 500:
            self.warnings.append(f"Item {index}: Product name too long, will be truncated")
        
        return item_valid
    
    def sanitize_product_data(self, products: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _sanitize_single_product(self, product: Dict[str, Any], index: int) -> Optional[Dict[str, Any]]:
        """
    ...
    def check_duplicates(self, products: List[Dict[str, Any]], supplier_name: str) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        existing_names = set(p.product_name.lower() for p in existing_products if p.product_name)
